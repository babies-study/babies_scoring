...
Retain observations that occured between ages 5-8 months or 5-9 months (depending on analysis goal)
...
lena_time_day <- 
  ...
  mutate(
    ...
    #calculation of N 5-minute segments will sometimes result in proportion values >1, correct to = 1,
...
lena_final_lf <-
  ...
  group_by(ID, day) %>% 
  mutate(
    duration = sum(duration_time_day)
  )
```
...
lena_final_wf <-
  ...
  gather(variable, value, age:day_type, duration) %>% 
...
lena_day1 <- 
  ...
  mutate(
    ...
    awc_hour_max = if_else(
      is.na(awc_hour_max_1), awc_hour_max_2, awc_hour_max_2
    ),
    ctc_hour_max = if_else(
      is.na(ctc_hour_max_1), ctc_hour_max_2, ctc_hour_max_2
    ),
    cvc_hour_max = if_else(
      is.na(cvc_hour_max_1), cvc_hour_max_2, cvc_hour_max_2
    ),
    day_type = if_else(
